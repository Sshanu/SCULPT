...
def evaluation(task, prompt, examples, predictor, args, metric="accuracy",base_index_list_index=0):
    ...
    predictions, evaluation_usage = compute_scores(task, prompts_exs_to_compute, predictor, args.max_threads, args.task,base_index_list_index)
    ...
    score = round(score, 2)
    return None, predictions, answers, score, evaluation_usage
...
def calculate_metrics_per_class(answers, predictions):
    ...
    precision_recall_dict["accuracy"] = round(accuracy, 2)
    precision_recall_dict['macro_f1'] = round(f1_macro, 2)
    precision_recall_dict['weighted_f1'] = round(f1_weighted, 2)
