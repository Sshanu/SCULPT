...
class SuccessiveHalvingEvaluator:
    ...
    num_prompts_per_round=10, samples_per_eval=5, max_threads=1, verbose=False, budget=None):
        ...
        for r in range(0, math.ceil(math.log2(n))):
            ...
            while True:
                try:
                    scores = scorer(predictor, S, sample, max_threads=max_threads, taskName=taskName)
                    break
    ...
    num_prompts_per_round=10, samples_per_eval=5, max_threads=1, verbose=False):
        ...
        with tqdm(total=len(idx2prompt), desc='sr') as pbar:
            while True:
                ...
                while True:
                    try:
                        scores = scorer(predictor, selected_prompts, selected_data, max_threads=max_threads, taskName=taskName)
                        break
                    except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError, urllib3.exceptions.MaxRetryError):
                        pass
...
class UCBBanditEvaluator:
    ...
    def __call__(self, prompts, exs, task, predictor, scorer, 
               rounds=40, num_prompts_per_round=10, samples_per_eval=5, max_threads=1, verbose=True, taskName=None):
        ...
        for ri in tqdm(range(rounds), desc=f'Evaluating {len(prompts)} prompts'):
            ...
            while True:
                try:
                    scores, token_usage_i = scorer(predictor, sampled_prompts, sampled_data, max_threads=max_threads, taskName=taskName)
                    break
                except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError, urllib3.exceptions.MaxRetryError):
                    pass
        ...
        return bandit_algo.get_scores().tolist(), token_usage
...
class BruteForceEvaluator:
    ...
    def __call__(self, prompts, exs, task, predictor, scorer,
rounds=40, num_prompts_per_round=10, c=2.0, samples_per_eval=5, max_threads=1, verbose=True, taskName=None):
        ...
        while True:
            try:
                scores, token_usage = scorer(predictor, prompts, eval_exs, max_threads=max_threads, taskName=taskName)
                break
            except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError, urllib3.exceptions.MaxRetryError):
                pass
